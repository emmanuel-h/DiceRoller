...
- `presentation/component/` — Compose components. `DiceImage` renders one pack drawable with `ContentScale.Fit`. `DiceImageSize` (enum: Inline/Small) sets the box it is fitted into. `DiceStepperChip` is one die type's count control: the artwork *is* the control, with its left half decrementing and its right half incrementing. `DiceResultDisplay` shows the roll as one wrapping line of `art + value + ×N` entries per die type, then a demoted total. `DiceColorSwatchRow` is the 12-dot color picker.
...
- `MainActivity` owns the single screen, laid out as four bands so a realistic pool fits in one view without scrolling: the color swatch row, a 3×2 grid of `DiceStepperChip`s, a flexible `DiceResultDisplay`, and a pinned bottom bar holding the roll button with the artwork attribution beneath it. There is no top app bar — the title was not worth its height. Only the result band scrolls, and only at pool sizes the design treats as extreme.
